# @agentxv2/payments — 独立项目集成部署指南

本文档说明如何把通用支付模块 `@agentxv2/payments` 集成到一个**全新的、与 AgentX 无关的项目**中，让该项目获得三种支付能力：
...
{
  "dependencies": {
    "@agentxv2/payments": "file:../payments"   // 按你的来源替换，见下表
...
| 场景 | 写法 |
| --- | --- |
| Monorepo / 本地开发 | `"@agentxv2/payments": "file:../payments"` |
| Git 仓库（打 tag 固定版本） | `"@agentxv2/payments": "git+https://github.com/<org>/payments.git#v0.2.0"` |
| 私有 npm registry | `"@agentxv2/payments": "^0.2.0"`（配合 `.npmrc`） |
...
> `rm -rf node_modules/@agentxv2/payments && npm install`
...
for f in node_modules/@agentxv2/payments/db/migrations/*.sql; do
...
// src/payments.ts
import { Pool } from 'pg'
import { PaymentsService, PgPaymentStore } from '@agentxv2/payments'
...
import { createPaymentsRouter } from '@agentxv2/payments/router'
...
```ts
import { PaymentsService, PaymentError, isPaymentError } from '@agentxv2/payments'
...
```ts
import type { PaymentStore, PaymentCredit } from '@agentxv2/payments'
...
```bash
cd node_modules/@agentxv2/payments && npm test
...
8. **file: 依赖刷新**：开发期用 `file:` 时，模块更新后必须 `rm -rf node_modules/@agentxv2/payments && npm install`（拷贝安装不自动刷新）。
