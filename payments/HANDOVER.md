# 通用支付引擎移交文档 — @agentxv2/payments

> **交接方**：AgentX（sftgroup）
> **接收方**：InfraX 通用平台团队
> **交接日期**：2026-08-07
> **包名**：`@agentxv2/payments`
> **npm 版本**：`0.2.0`（latest，2026-08-07 发布，含 P0-P4 全部能力）
> **仓库**：本仓库（独立于 AgentX 业务仓库）
...
1. **零 AgentX 依赖**：`dependencies` 仅 `pg` + `viem`；`express` 是 optional peer（仅 `@agentxv2/payments/router` 需要）。`src` 中不允许出现任何 AgentX 业务 token（`fiat_subscriptions`、`x402_*`、`agentId` 等），有自动化断言（见 §8 解耦验证）。
...
```
payments/
├── package.json            # @agentxv2/payments v0.2.0, deps: pg + viem
├── tsconfig.json
...
- 部署：`payments build`（tsc）→ 宿主 `npm install`（file: 为拷贝安装，改模块后需重装刷新）→ 宿主 build → 重启
...
3. **`file:` 依赖是拷贝安装**（npm），非 symlink：改模块后宿主必须 `rm -rf node_modules/@agentxv2/payments && npm install`，否则 typecheck 用旧 API。
...
# 3. 发布（npm 账号：sftgroup）
npm publish --access public
# 4. 验证
npm view @agentxv2/payments dist-tags   # latest 指向新版本
```
