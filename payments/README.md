...
{
  "dependencies": {
    "@agentxv2/payments": "file:../payments" // 或 git / 私有 registry，见下
...
| 私有 npm registry | `"@agentxv2/payments": "^0.2.0"`（配合 `.npmrc` 指向 registry） |
...
`@agentxv2/payments/router` 提供了覆盖全部端点（`/info` `/price` `/checkout` `/verify` `/webhook` `/balance` `/access`）的现成 router，挂载即用：
...
```ts
import { PaymentsService } from '@agentxv2/payments'
...
| `createPaymentsRouter` | 现成 Express router（`@agentxv2/payments/router`） |
...
```
payments/
├── package.json            # @agentxv2/payments, deps: pg + viem
├── tsconfig.json
