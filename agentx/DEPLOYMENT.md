...
> 三端代码同步：本地 workspace ↔ 测试服务器 (43.156.78.59) ↔ GitHub
> 
> 最后更新: 2026-07-14 03:53 GMT+8 (25/25 P0-P3 完成, P3 #22 链切换 UI done, 仅剩生产部署)
> 
...
| 组件 | 状态 | 详情 |
|------|:--:|------|
| Sepolia 合约 (6 core) | ✅ | 全部部署 + Blockscout 验证 |
| OxaChain L1 合约 (6 core) | ✅ | 2026-07-14 DeployOxaChainFull.s.sol via-ir |
| SDK npm | ✅ | `@agentxv2/sdk@0.5.4` (36 files, 11 modules, CJS+ESM+DTS) |
| 前端测试服务器 | ✅ | http://43.156.78.59:8080 (5 pages 200 OK) |
| 前端 OxaChain 支持 | ✅ | .env + wagmi + WalletConnect + NetworkSwitcher chain name |
| 项目 README | ✅ | README.md — full architecture + code structure + API + MCP |
| 全链路测试 | ✅ | 18/19 PASS — REAL_WORLD_TEST_REPORT.md |
...
| 仓库 | 用途 | 最新 Tag |
|------|------|----------|
| [sftgroup/Agentx](https://github.com/sftgroup/Agentx) | AgentX 全平台 monorepo（主） | `v0.7.0-20260714` |
| [sftgroup/erc8004](https://github.com/sftgroup/erc8004) | ERC-8004 原始合约 + AgentX 备份 | `v1.0.0-20260714` |
...
```bash
sshpass -p 'REMOVED_CREDENTIAL' ssh ubuntu@43.156.78.59 '
...
```bash
# 上传单个文件
sshpass -p 'REMOVED_CREDENTIAL' scp app/xxx/page.tsx ubuntu@43.156.78.59:/home/ubuntu/agentx-platform/app/xxx/page.tsx
...
for f in $(find agentx/frontend/app -name 'page.tsx' | sort); do
  ...
  remote_md5=$(sshpass -p 'REMOVED_CREDENTIAL' ssh ubuntu@43.156.78.59 "md5sum /home/ubuntu/agentx-platform/app/$rel | cut -d' ' -f1")
...
### 2026-07-14 — OxaChain L1 全链部署 + SDK + 文档 + 测试

| SHA | 描述 |
|-----|------|
| `6800da2` | docs: comprehensive project README — architecture, code structure, API, MCP, encryption pipeline |
| — | fix(sdk): tsup + postbuild.js missing endpoint/configuration entries → v0.5.4 |
| — | fix(frontend): Studio root page.tsx redirect, Turbopack root config, test server deploy |
| — | test: 18/19 PASS — frontend E2E + dual-chain bytecode + DApp chain config |
| `1ea741f` | feat(frontend): OxaChain L1 wagmi config + .env dual-chain addresses (6 contracts each) |
| `b1570a0` | feat(sdk): MultiEndpointClient + ConfigurationClient modules → v0.5.1 |
| `65795eb` | feat(contracts): deploy 6 core contracts to OxaChain L1 — DeployOxaChainFull.s.sol via-ir |
