...
| `X-Llm-Model` | 可选 | 与 `X-Llm-Api-Key` 配套的模型名（如 `deepseek-v4-pro`；缺省 `gpt-4o`） |
...
> 完整服务端协议见仓库 [CONVERSATION_SERVICE.md](CONVERSATION_SERVICE.md)，SDK 用法见 [INTEGRATION.md](INTEGRATION.md)（`ConversationClient`，v0.8.0）。
