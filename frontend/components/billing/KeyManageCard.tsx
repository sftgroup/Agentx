...
export function KeyManageCard({ accessToken }: { accessToken: string }) {
  ...
  const rotate = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t('billing.rotateFailed'))
      setNewKey(data.api_key!)
    } catch (err) {
      setError(err instanceof Error ? err.message : t('billing.rotateFailed'))
    } finally {
      setRotating(false)
    }
  }
    ...
    <div className="glass-card p-6 rounded-2xl space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-sm flex items-center gap-2">
          <KeyRound className="w-4 h-4 text-accent-purple" /> {t('billing.apiKeyTitle')}
        </h3>
        <span className="text-[11px] px-2 py-0.5 rounded-full bg-green-500/10 text-green-400 border border-green-500/20 flex items-center gap-1">
          <ShieldCheck className="w-3 h-3" /> {t('billing.shaHashed')}
        </span>
      </div>
      ...
      {newKey ? (
        <div className="space-y-3">
          ...
          <div className="rounded-xl bg-yellow-500/10 border border-yellow-500/20 p-3 flex items-start gap-2 text-xs text-yellow-300">
            ...
            <span>{t('billing.oldKeyInvalid')}</span>
          </div>
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            >
              ...
              {copied ? t('billing.copied') : t('billing.copyNewKey')}
            </button>
            <button
            ...
            >
              ...
              {saved ? t('billing.saved') : t('billing.savedIt')}
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          >
            ...
            {rotating ? t('billing.rotating') : t('billing.rotateKey')}
          </button>
        </div>
      )}
    </div>
