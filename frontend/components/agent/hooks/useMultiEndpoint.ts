...
// R7 拆分：类型、常量与纯工具函数移入独立模块
import { MULTI_ENDPOINT_ADDRESS, tupleToEndpointStats } from './multi-endpoint-types'
import type { Endpoint, ProtocolConfig, EndpointStats, UseMultiEndpointReturn } from './multi-endpoint-types'

// 保持对外 API 兼容（外部消费方从本文件导入类型）
export type { Endpoint, ProtocolConfig, EndpointStats, UseMultiEndpointReturn } from './multi-endpoint-types'
