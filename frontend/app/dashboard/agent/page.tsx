    ...
    return (
      <AppLayout>
        <div className="max-w-4xl mx-auto text-center py-20">
          ...
          <h2 className="heading-md mb-3">{t('dashboard.connectTitle')}</h2>
          <p className="body text-text-muted">{t('dashboard.connectDesc')}</p>
        </div>
      </AppLayout>
    )
  }
      ...
      <div className="max-w-5xl mx-auto py-8 px-6 space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="body text-text-secondary mt-1">{t('dashboard.desc')}</p>
          </div>
          <Link href="/studio" className="btn-primary text-sm px-5 py-2.5"><Plus className="w-4 h-4" /> {t('dashboard.newAgent')}</Link>
        </div>

        <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard label={t('dashboard.myAgents')} value={agentsArr.length} icon={Brain} color="text-accent-purple" />
          <StatCard label={t('dashboard.activeSubs')} value={activeSubs} icon={Users} color="text-accent-cyan" trend="+12%" trendUp />
          <StatCard label={t('dashboard.totalRevenue')} value="— ETH" icon={DollarSign} color="text-yellow-400" />
          <StatCard label={t('dashboard.tasksProcessed')} value="—" icon={Zap} color="text-accent-purple" />
        </div>

        <div>
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2"><Brain className="w-4 h-4 text-accent-purple" /> {t('dashboard.myAgents')}</h2>
          {agentsArr.length === 0 ? (
            <div className="glass-card p-12 text-center">
              <Brain className="w-12 h-12 text-text-muted mx-auto mb-3 opacity-30" />
              <p className="text-text-secondary mb-4">{t('dashboard.createFirst')}</p>
              <Link href="/studio" className="btn-primary text-sm"><Plus className="w-4 h-4" /> {t('dashboard.createAgent')}</Link>
            </div>
          ) : (
                ...
                <div key={id} className="glass-card glass-card-hover p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-text-muted">{t('dashboard.tokenIdLabel')} {id}</div>
                    </div>
                  </div>
                  <div className="flex items-center justify-end mt-3">
                    <Link href={`/marketplace/agent/${id}`} className="text-xs text-accent-cyan flex items-center gap-1 hover:underline">{t('dashboard.view')} <ArrowRight className="w-3 h-3" /></Link>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            { label: t('dashboard.agentStudio'), desc: t('dashboard.agentStudioDesc'), href: '/studio', icon: Plus, color: 'text-accent-purple' },
            { label: t('dashboard.settings'), desc: t('dashboard.settingsDesc'), href: '/user/settings', icon: Settings, color: 'text-accent-cyan' },
            { label: t('dashboard.docs'), desc: t('dashboard.docsDesc'), href: 'https://github.com/sftgroup/Agentx', icon: FileText, color: 'text-text-muted' },
          ].map(a => (
