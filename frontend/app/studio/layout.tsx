// app/studio/layout.tsx — Studio shell with Provider + step indicator
'use client'

import Link from 'next/link'
import { AppLayout } from '@/components/layout/AppLayout'
import { StudioProvider } from '@/components/studio/StudioContext'
import { StepIndicator } from '@/components/studio/StepIndicator'
import { StudioHeader } from '@/components/studio/StudioHeader'
import { useCurrentStep } from '@/components/studio/stepFromPath'

// Ensure Next.js pre-renders all sub-routes
const STUDIO_STEPS = ['/studio/basics', '/studio/skills', '/studio/encrypt', '/studio/publish']

export default function StudioLayout({ children }: { children: React.ReactNode }) {
  const step = useCurrentStep()

  return (
    <StudioProvider>
      {/* Hidden preload links — ensures Next.js includes sub-pages at build */}
      <div aria-hidden="true" className="hidden">
        {STUDIO_STEPS.map(s => (
          <Link key={s} href={s} prefetch>{s}</Link>
        ))}
      </div>
      <AppLayout>
        <div className="max-w-4xl mx-auto py-8 px-6 space-y-6">
          <StudioHeader />
          <StepIndicator current={step} />
          {children}
        </div>
      </AppLayout>
    </StudioProvider>
  )
}
