...
> 适用版本：`@agentxv2/sdk >= 0.11.7`（含 `AgentCategory` / `AGENT_CATEGORIES` / 三轨订阅支付 / 平台套餐购买 `TenantPlanPayments` / 服务端 A2A 按次付费 + `BillingClient` 余额预检 / `AgentWalletConfig` agent 自主钱包（MPC）管理 · A2A 代付 / 用户钱包签名上链编排）
