...
> 本文件与 `docs/test-cases-consumer-c-end.md`（337 条逐条用例库）互补：后者是**离散断言**，本文件是**旅程式完整操作路径**——每一条 Journey 模拟真实用户在一个页面上的一次完整操作序列（点击/输入/等待/校验），且**每条正常路径都配一条对应的"用户会犯的错误"路径**（误操作、输入错误、中断、重复点击、余额不足、签名拒绝等）。
>
