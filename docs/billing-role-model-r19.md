...
> 状态：方案定稿（2026-08-11）· 待实施 · 关联 PROGRESS.md「R19」条目
...
- **D2 申请模式去留**：保留 `/developer/apply` 作为「人工审批/白名单」通道，自助钱包登录为默认；是否下线由运营决策（待定 T1）。
...
| 阶段 | 内容 | 关键点 |
|---|---|---|
| R19.1 | B 端钱包登录 + 自动建租户 + 自动发 key | auth 扩展 kind=partner；key 明文→hash 化策略评估 |
...
| # | 决策 | 选项 |
|---|---|---|
| T1 | 申请表单去留 | 保留为人工通道 / 下线 |
| T2 | 存量 key 迁移 | 迁移（双写期）/ 仅新 key hash 化 |
| T3 | 自助注册防滥用 | 单钱包单租户 / 频率限制 / 邀请码 |
| T4 | B 端套餐定价结构 | 订阅制（套餐）/ 按量预充值 / 两者并行 |
