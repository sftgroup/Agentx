...
- **待办**：均为外部前提——R4（Stripe 业务方凭据；R5 已随 R19 完成），见下
  - R1 ✅ 已完成（2026-08-06 · commit `0f5c30d`；SDK 0.8.7 已发布 npm）
  - R2 ✅ 已完成（2026-08-06 · 集成测试补 task 并行链路，生产 28/28 通过）
  - R4 = 待外部前提任务（需业务方提供 Stripe 商户凭据）
  - R5 ✅ 已完成（2026-08-12 经 R19 生产启用 x402 门卫 + 链上记账，见下）
...
**R5 x402 支付门卫启用** —— 优先级：中 · ✅ 已完成（2026-08-12 随 R19 落地并生产启用）
- 来源：P2-3（原 ⏸ 待 `X402_ENABLED=true` + `X402_PAY_TO`）
- 实施要点：配置两个环境变量 → 验证 `POST /agent/runs` 402 门卫（返回 `x-price/x-pay-to/x-network`）→ 支付后余额账本记账
- 验收标准：未支付请求 402 + 响应头齐全，支付后请求放行且账本正确
- 实现记录：**2026-08-12 随 R19 生产启用**——生产 `.env` 已配置 `X402_ENABLED=true` + `X402_PAY_TO=0x70997970C51812dc3A010C7d01b50e0d17dc79C8`（统一环境文件 `scripts/local-payments` 地址）；链上充值记账（`/x402/verify` 余额 0.5 OXA）、A2A 按次扣费（未拥有 agent 66 委派扣 0.001 OXA + `a2a_pay_log` 幂等）均已实测。R5 由此不再属外部前提（详见「## R19」）。
