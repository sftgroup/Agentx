#!/usr/bin/env bash
# =============================================================================
# @agentxv2/payments — 解耦性验证（standalone / version-A shape）
#
# 与 run.sh（验证 Gateway 内嵌形态）不同，本脚本验证「通用模块独立可运行」：
...
#   - 跑 decouple-test.mjs —— 该脚本只 import @agentxv2/payments 及其自有依赖
...
echo "==> [6/7] 跑解耦性验证（只 import @agentxv2/payments + 自有依赖）"
(
  cd "$DIR"
